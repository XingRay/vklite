...
namespace vklite {
    ...
    DescriptorConfigure &DescriptorConfigure::addDescriptorSetConfigure(const std::function<void(DescriptorSetConfigure &)>& configure) {
    ...
    uint32_t DescriptorConfigure::getDescriptorSetCount() const {
        return static_cast<uint32_t>(mDescriptorSetConfigures.size());
    }
