...
namespace vklite {
    ...
    void HostVisibleBuffer::updateBuffer(const void *data, vk::DeviceSize size, vk::DeviceSize offset, vk::MemoryMapFlags flags) const {
        mCombinedMemoryBuffer.getDeviceMemory().updateBuffer(data, size, offset, flags);
    }

    void HostVisibleBuffer::updateBuffer(const void *data, vk::DeviceSize size) const {
